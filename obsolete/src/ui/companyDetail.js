...
export async function renderCompanyDetail(container, id) {
  ...
  try {
    ...
    if (error || !company) {
      container.innerHTML = '<div class="error">Company not found. <a href="#/crm/companies">Back to list</a></div>';
      return;
    }
    ...
    const logsHtml = companyLogs.length > 0
      ? companyLogs.map(log => {
          ...
          return `<div class="log-entry">
            ...
            <a href="#/crm/contacts/${log.contact_id}" class="log-tag">[${contactName}]</a>
      ...
      <div class="detail-page">
        <div class="detail-header">
          <div class="detail-toolbar">
            <a href="#/crm/companies" class="btn btn-back">&larr; Back</a>
        ...
        <div class="compact-list">
          ${contactsWithTemp.map(c => `
            <div class="compact-list-item clickable-row ${c.temp.css}" data-href="#/crm/contacts/${c.id}">
        ...
        <div class="compact-list">
          ${projectList.map(d => `
            <div class="compact-list-item clickable-row" data-href="#/crm/projects/${d.id}">
    ...
    container.querySelector('#delete-company').addEventListener('click', async () => {
      await deleteWithUndo('companies', company, `"${company.name}"`,
        () => { window.location.hash = '#/crm/companies'; },
        () => { window.location.hash = `#/crm/companies/${id}`; }
      );
    });
