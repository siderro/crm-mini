  ...
  try {
    ...
    container.innerHTML = `
      <div class="page-header">
        ...
        <div class="header-actions">
          ...
          <a href="#/crm/contacts/new" class="btn btn-primary">+ New Contact</a>
        </div>
      </div>
    ...
    container.querySelectorAll('.clickable-row').forEach(row => {
      row.addEventListener('click', () => {
        window.location.hash = `#/crm/contacts/${row.dataset.id}`;
      });
    });
...
function renderGroupedContacts(linkedToOpenDeals, others, lastLogMap) {
  ...
  if (linkedToOpenDeals.length === 0 && others.length === 0) {
    html = '<div class="empty-state">No contacts. <a href="#/crm/contacts/new">Create first</a>.</div>';
  }
