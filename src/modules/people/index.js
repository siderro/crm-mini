...
function superadminEmail() {
  return SUPERADMIN_EMAIL.trim().toLowerCase();
}
...
async function load(body) {
  ...
  if (!users.length) {
    body.innerHTML = `<div class="empty-state">Zatím se nikdo nepřihlásil.</div>`;
    return;
  }
  ...
  for (const u of users) {
    ...
    const badge = isSuperadmin
      ...
      : (count ? '' : `<span class="status status-frozen">čeká</span>`);
    ...
    <tr class="people-detail"><td colspan="5">
      <div class="people-panels">
        <div class="people-panel">
          <h2 class="people-panel-head">Moduly</h2>
          <div class="people-modules"></div>
        </div>
        <div class="people-panel">
          <h2 class="people-panel-head">Hodinová sazba</h2>
...
async function renderModules(el, row, body) {
  ...
  el.innerHTML = isSuperadmin
    ? `${rows}<p class="muted people-note">Super admin má všechno vždycky, práva plynou z konfigurace.</p>`
    : rows;
...
async function loadRates(el, email, body) {
  ...
  el.innerHTML = (list.length ? rows : `<div class="empty-state">Zatím žádná sazba.</div>`) + `
    <div class="rate-add">
  ...
  el.onclick = async (e) => {
    ...
    if (act === 'add') {
      ...
      if (!saved) return;   // nevyplněné částky prostě neuložíme
    } else {
  ...
  // Oprava existující verze se ukládá při opuštění pole.
  el.onchange = async (e) => {
    const field = e.target.dataset.field;
    if (!field) return;
    const id = e.target.closest('.rate-version').dataset.id;

    if (field === 'type') {
      // Přepnutí typu potřebuje i chybějící pole, jinak by se neuložilo.
      const record = (await getRates(email)).find((r) => r.id === id);
      const patched = { ...record, type: e.target.value };
      if (patched.type === 'monthly' && patched.monthly_hours == null) {
        patched.monthly_hours = DEFAULT_MONTHLY_HOURS;
      }
      await updateRate(id, patched);
    } else {
      await updateRate(id, { [field]: e.target.value });
    }
    await load(body);
    reopen(body, email);
  };
